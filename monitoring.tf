//File landed in expected time frame.
//Ec2 instance spin up
//Multiple files landed
//Instance shut down
//EMR cluster spin up
//Data added to published bucket

resource "aws_cloudwatch_event_rule" "instance_terminated_with_errors_rule" {
  name          = "data_ingress_instance_terminated_with_errors_rule"
  description   = "Sends failed message to slack when an instance terminates with errors"
  event_pattern = <<EOF
{
  "source": [
    "aws.emr"
