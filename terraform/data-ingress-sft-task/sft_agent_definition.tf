...
data "template_file" "sft_agent_receiver_definition" {
  ...
  vars = {
    ...
    cpu                = var.task_definition_cpu[var.environment]
    image_url          = format("%s:%s", "${var.account[var.management_account[var.environment]]}.dkr.ecr.${var.region}.amazonaws.com/${var.ecr_repository_name}", "${var.sft_agent_version}")
    memory             = var.task_definition_memory[var.environment]
