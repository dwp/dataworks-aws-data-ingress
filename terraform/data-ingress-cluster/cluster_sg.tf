...
resource "aws_security_group_rule" "data_ingress_host_outbound_tanium_1" {
  description              = "Data ingress host outbound port 1 to Tanium"
  type                     = "egress"
  from_port                = var.tanium_port_1
  to_port                  = var.tanium_port_1
  protocol                 = "tcp"
  source_security_group_id = var.tanium_service_endpoint_sg
  security_group_id        = aws_security_group.data_ingress_server.id
}

resource "aws_security_group_rule" "data_ingress_host_outbound_tanium_2" {
  description              = "Data ingress host outbound port 2 to Tanium"
  type                     = "egress"
...
resource "aws_security_group_rule" "data_ingress_host_inbound_tanium_1" {
  description              = "Data ingress host inbound port 1 from Tanium"
  type                     = "ingress"
  from_port                = var.tanium_port_1
  to_port                  = var.tanium_port_1
  protocol                 = "tcp"
  security_group_id        = var.tanium_service_endpoint_sg
  source_security_group_id = aws_security_group.data_ingress_server.id
}

resource "aws_security_group_rule" "data_ingress_host_inbound_tanium_2" {
  description              = "Data ingress host inbound port 2 from Tanium"
  type                     = "ingress"
  from_port                = var.tanium_port_2
  to_port                  = var.tanium_port_2
  protocol                 = "tcp"
