...
resource "aws_autoscaling_group" "data_ingress_server" {
  name = var.autoscaling_group_name
  min_size              = var.asg_instance_count.min[local.environment]
  max_size              = var.asg_instance_count.max[local.environment]
  desired_capacity      = var.asg_instance_count.desired[local.environment]
